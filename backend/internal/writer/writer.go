...
// optional overrides) and its whole folder, so the new instance starts as a
// full copy of an existing one — exactly what a user would do by hand with
// cp -r on the repository.
func CloneInstance(root, from, newName string, patch InstancePatch) (model.Instance, error) {
	...
	err := mutateRegistry(root, func(reg *model.InstanceRegistry) error {
		...
		clone := *src
		clone.Name = newName
		// The clone's folder follows the source's convention: the parent
		// directory keeps its place, only the leaf is renamed.
		clone.Folder = filepath.ToSlash(filepath.Join(filepath.Dir(srcFolder), newName))
		if clone.Labels != nil {
	...
	if err := copyDir(filepath.Join(root, srcFolder), filepath.Join(root, created.FolderOrDefault())); err != nil {
		return model.Instance{}, fmt.Errorf("copy instance folder: %w", err)
	}
	return created, nil
}

// copyDir recursively copies src to dst (regular files only).
