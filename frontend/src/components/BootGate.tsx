...
export default function BootGate({ children }: { children: React.ReactNode }) {
  ...
  if (q.isError) return <ServiceUnavailable onRetry={() => q.refetch()} retrying={q.isFetching} />;
...
// ServiceUnavailable is the full-page state for "the service isn't answering".
// It says so in plain words, keeps retrying on its own with a visible count, and
// offers the manual retry - no stack traces, no status codes, no dead end.
function ServiceUnavailable({ onRetry, retrying }: { onRetry: () => void; retrying: boolean }) {
    ...
    <div className="boot-screen">
      <div className="boot-card">
        ...
        <Typography.Title level={4} style={{ margin: "10px 0 0" }}>
          {brand.appName} can&rsquo;t reach its service
        </Typography.Title>
        <Typography.Text type="secondary" style={{ fontSize: 13.5 }}>
          The service isn&rsquo;t responding right now. It may be starting up, restarting, or briefly
          under maintenance. Nothing you have saved is affected - your configuration lives in Git.
        </Typography.Text>
